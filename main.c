...
int Manager_Input(short arr[], int timeTaken_Sort_Arr[], int stat[])
{
	...
	while (select != 27)
	{
		...
		else if ((select == 8) || ((select >= 48) && (select <= 57)))
		{
			...
			{
				count /= 10;
				strcpy(count_s, _itoa(count, count_s, 10));
			}
			else
			{
...
const char* Calc_ReqMem(int count_Int, int stat[])
{
	...
	{
		strcpy(count_s, _itoa((count * multiplier), count_s, 10));
		strcat(count_s, " Byte");
	}
...
const char* Calc_Comma000(int count)
{
	if (count > 999)
	{
		...
		for (int i = dot_count; i > 0; i--)
		{
			...
			{
				if ((strlen(_itoa(temp_count, temp_count_s, 10)) < 3) && (i != dot_count))
				{
					int zeroCount = 3 - strlen(_itoa(temp_count, temp_count_s, 10));
					...
					strcat(count_s, _itoa(temp_count, temp_count_s, 10));
				}
				else
				{
					strcat(count_s, _itoa(temp_count, temp_count_s, 10));
		...
		{
			int zeroCount = 3 - strlen(_itoa(temp_count % 1000, temp_count_s, 10));

			for (int i = 0; i < zeroCount; i++)
			{
				strcat(count_s, "0");
			}
			strcat(count_s, _itoa(temp_count % 1000, temp_count_s, 10));
		}
